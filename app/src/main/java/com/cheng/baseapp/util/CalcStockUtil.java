...
//                    low=Float.valueOf(datas.getK().get(i).low);
//                }
//                volume+=Integer.valueOf(datas.getK().get(i).volume);
//                if (j==num-1 || i==max-1){
...
//                    data.high=high+"";
//                    data.low=low+"";
////                    data.day=datas.getK().get(i).day;
//                    data.time=datas.getK().get(i).time;
//                    data.volume=volume/num+"";
...
//    }
    ...
    public KChartBean transformTime(KChartBean chartlist, int num){
        KChartBean convertData=new KChartBean();
        int max=chartlist.getK().size();
        if (num!=1){
            String open="";
            String close="";
            float high=0;//最高价
            ...
            for (int i = 0,j=0; i <max; i++) {
                if (j==0){
                    open= chartlist.getK().get(i).open;
                    high=Float.valueOf(chartlist.getK().get(i).high);
                    low=Float.valueOf(chartlist.getK().get(i).low);
                }
                if (high < Float.valueOf(chartlist.getK().get(i).high)){
                    high=Float.valueOf(chartlist.getK().get(i).high);
                }
                if (low > Float.valueOf(chartlist.getK().get(i).low)){
                    low=Float.valueOf(chartlist.getK().get(i).low);
                }
                volume+=Integer.valueOf(chartlist.getK().get(i).volume);
                if (j==num-1 || i==max-1){
                    close= chartlist.getK().get(i).close;
                    KChartData data=new KChartData();
                    data.open=open;
                    data.close=close;
                    data.high=high+"";
                    data.low=low+"";
//                    data.day=datas.getK().get(i).day;
                    data.time=chartlist.getK().get(i).time;
                    data.volume=volume/num+"";
                    convertData.getK().add(data);//加入到转换后的集合中
                    volume=0;
                    j=0;
    ...
    public void CalcEMV(List<KChartData> data,List<Entry> EMVData, List<Entry> MAEMVData){
        ...
        for (int i = 0; i < data.size(); i++) {
            ...
            float em=0;
            if (Float.valueOf(data.get(i).volume)!=0){
                em=(a-b)*c/Float.valueOf(data.get(i).volume);
            }
            EMData.add(em);
    ...
    public float OBV(int time, int index, List<KChartData> data)
    {
        ...
        for (; startIndex <= index; startIndex++)
        {
            ...
            {
                all += Float.valueOf(data.get(index).volume);
            }
            else
            {
                all -= Float.valueOf(data.get(index).volume);
    ...
    public float VR(int time,int index,List<KChartData> data)
    {
        ...
        for (; startIndex <= index; startIndex++)
        {
            ...
            float vol = Float.valueOf(data.get(startIndex).volume);
