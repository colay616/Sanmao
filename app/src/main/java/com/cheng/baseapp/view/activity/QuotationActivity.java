...
import com.cheng.baseapp.adapter.OrderAdapter;
import com.cheng.baseapp.adapter.TabAdapter;
import com.cheng.baseapp.api.Const;
import com.cheng.baseapp.api.CoupleChartGestureListener;
import com.cheng.baseapp.bean.KChartBean;
import com.cheng.baseapp.bean.KChartData;
import com.cheng.baseapp.bean.OrderData;
import com.cheng.baseapp.bean.StockOneMinData;
import com.cheng.baseapp.bean.TodayBean;
import com.cheng.baseapp.model.ChartModel;
import com.cheng.baseapp.util.BaseUtil;
import com.cheng.baseapp.util.CalcStockUtil;
import com.cheng.baseapp.util.HttpUtil;
import com.cheng.baseapp.util.ThreadManager;
import com.cheng.baseapp.view.widget.KCombinedChart;
import com.cheng.baseapp.view.widget.MyMarrkerViewBottom;
import com.cheng.baseapp.view.widget.MyMarrkerViewLeft;
import com.cheng.baseapp.view.widget.MyXFormatter;
...
//    private String goodsCode;
    ...
    private int Mode=7;
    private int sum;
//    private String mf="5";//2为分钟 1为日
    private String mf="1day" ;//2为分钟 1为日
...
//                        KChartBean bean=mStockUtil.transformTime(mDatas,stateTime);//转换后的数据
                        ...
                        System.out.println("mdata水水水水水水水");
                    }
                    break;
...
//        Log.e("@#",goodsId+"--"+goodsName+"--"+goodsCode);
        tv_goodsName.setText(shares.toUpperCase());
//        mf="5";
        mf="1day";
        mTask = new ChartTask();
...
//    private StockOneMinData initCharData(String symbol, String period){
    private void initCharData(String symbol, String period){
...
//                    "stocklist.json?symbol=%s&period=%s&one_min=1", symbol,period);
//
//
...
//        }
        ...
        mModel.getKData(this, symbol,period, new ChartModel.ResultKListener() {

            @Override
            public void ResultData(boolean isSuccess, KChartBean chartlist) {
                if (isSuccess){
                    Message message=new Message();
                    message.what= Const.STATE_DATA_SUCCESS;
                    message.obj=chartlist;
                    mHandler.sendMessage(message);

                }else {
                    System.out.println("lalallalalallalalala");
                }
            }
        });
...
//                        StockOneMinData oneMinData = initCharData(shares.toUpperCase(),mf);


                        initCharData(shares.toUpperCase(),mf);
                    }
//                    initTodayData("20");
                    isRefresh=false;
                }
...
//            bottomEntry.add(new BarEntry(i,vol));
            bottomEntry.add(new BarEntry(i,Float.valueOf(datas.get(i).volume)));
...
//        bottomchart.setExtraRightOffset(num);//对齐设置    10000/-12  100/-20
    ...
    public void drawVol(KChartBean bean , String[] time){
        ...
        List<String> ydata=new ArrayList<>();
        for (int i = 0; i < bean.getK().size(); i++) {
            ydata.add(bean.getK().get(i).volume);
        }
